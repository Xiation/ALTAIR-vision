...
def build_lines_from_nodes(major_nodes, radius):
    """
    Build lines from major nodes based on proximity and alignment.
    :param major_nodes: List of major nodes (x, y, weight).
    :param radius: Maximum distance for connectivity.
    :return: List of lines (each line is a list of connected nodes).
    """
    lines = []
    used = set()
...
def detect_intersections(lines):
    """
    Detect intersections (L, T, X shapes) from lines.
    :param lines: List of lines (each line is a list of connected nodes).
    :return: Dictionary with 'L', 'T', and 'X' intersections.
    """
